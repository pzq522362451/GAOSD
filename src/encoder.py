...
if GAN1D:
    root_best = '../best1d'
    root_data = '../data/Printer3D_time'
    from model_1d import BiGAN_model
else:
    root_best = '../best'
    root_data = '../data/Printer3D_spectrum'
    from model import BiGAN_model

faults = ['M','N','O','P']#['A','B','C','D','E','F','G','H','I','J','K','L','P']
sensors = [6,7,8]
for fault in faults:
    path_out = '../results/data_encoded_f_'+fault+'.pkl'
    train_encoded = []
    test_encoded = []
    for sensor in sensors:
        best_log_dir = root_best+'/sensor'+str(sensor)

        data_dir = root_data+'/dataset_'+fault+'_sensor' + str(sensor) + '.pkl'
